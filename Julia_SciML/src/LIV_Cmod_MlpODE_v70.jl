# ============================================================================
# MlpODE_v70.jl — Pure neural ODE for plasma current and inductance
...
# LIV_Cmod_RomeroNN_v70.jl, which implements the physics-informed
...
#   V_dot diagnostic R² (R² ≈ 0.0075 in v69, where V was unsupervised)
#   improves when V is in the loss. RomeroNN_v70 ran first and showed
#   no meaningful improvement (R² 0.0066 → 0.0141, both effectively zero,
#   with degraded Ip tracking as the cost of V supervision); MlpODE_v70
#   completes the cross-architecture picture.
#
...
function loss_multishot(θ)
    ...
    for sid in train_shot_ids
        ...
        # Pure-NN ODE RHS for THIS shot. Captures `spl_s` and `θ` in its
        # closure. All three derivatives produced by the NN; no Romero
        # physics terms (cf. RomeroNN's hybrid closure).
        # NOTE: must stay in sync with `dudt_mlp_diag` (used by the
        # diagnostic plotting at end of script).
...
#
# NOTE: Unlike RomeroNN_v70, MlpODE has no κ_empirical or τ to save —
...
#
# NOTE: Unlike RomeroNN_v70's evaluate_split_endpoint, the dudt_eval_phil
...
CSV.write("fullshot_mape_TEST_MlpODE_v70.csv", fullshot_test_df)
println("Saved per-shot full-shot MAPE (TEST) to fullshot_mape_TEST_MlpODE_v70.csv")
...
CSV.write("endpoint_mape_TEST_MlpODE_v70.csv", endpoint_test_df)
println("Saved per-episode endpoint MAPE (TEST) to endpoint_mape_TEST_MlpODE_v70.csv")
...
#     Unlike RomeroNN_v70, all three derivatives here come from the NN
...
# is necessary because RomeroNN_v70 hit "maxit reached, s too small"
...
CSV.write("derivatives_MlpODE_v70.csv", derivatives_df)
println("Saved derivatives CSV: $(nrow(derivatives_df)) rows × $(ncol(derivatives_df)) cols → derivatives_MlpODE_v70.csv")
...
CSV.write("loss_history_MlpODE_v70.csv", loss_df)
println("Saved training loss history to loss_history_MlpODE_v70.csv")

val_df = DataFrame(
    iter     = [vh[1] for vh in val_history],
    val_loss = [vh[2] for vh in val_history],
)
CSV.write("val_loss_history_MlpODE_v70.csv", val_df)
println("Saved validation loss history to val_loss_history_MlpODE_v70.csv")
...
plot!(plt_loss, val_df.iter, val_df.val_loss,
      ...
      seriestype = :scatter,
      ms    = 4)
savefig(plt_loss, "loss_history_MlpODE_v70.png")
println("Saved loss-history plot to loss_history_MlpODE_v70.png")
...
CSV.write("predictions_MlpODE_v70.csv", results_df)
println("Saved diagnostic predictions to predictions_MlpODE_v70.csv")
...
CSV.write("MlpODE_predictions_hard_shot_v70.csv", hard_df)
println("Saved hard-shot evaluation to MlpODE_predictions_hard_shot_v70.csv")
...
savefig(plt_li, "li_MlpODE_comparison_v70.png")
...
savefig(plt_ip, "ip_MlpODE_comparison_v70.png")
...
savefig(plt_V, "V_MlpODE_comparison_v70.png")
