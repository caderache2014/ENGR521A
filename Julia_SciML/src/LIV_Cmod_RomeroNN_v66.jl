...
liip_norm    = T(0.85)   # added for v66
κ_empirical = T(0.98)
τ           = T(1.25)
...
# ============================================================================
# v66: PRE-COMPUTED TRAINING SEGMENT LIST FOR MINIBATCH SAMPLING
#
# To match Wang's batch_size=512 methodology, we pre-compute a flat list
# of all training segments (matching loss_multishot's segmentation logic
# of segment_length=10 with overlap-by-1). The training loss then samples
# a random subset of size BATCH_SIZE from this list per gradient step,
# rather than iterating over all segments every step.
#
# With ~342 train shots × ~3 segments each, total ~1000 segments, so
# BATCH_SIZE=512 means roughly half of all segments are seen per step.
#
# Each entry: NamedTuple with sid, i_start, i_end. The actual data lookup
# (d.li, d.ip_MA, d.V, spl_s, etc.) happens inside loss_multishot using
# these indices.
# ============================================================================
const BATCH_SIZE     = 512
const SEGMENT_LENGTH = 10

all_train_segments = NamedTuple{(:sid, :i_start, :i_end), Tuple{Int64, Int, Int}}[]
for sid in train_shot_ids
    d = shot_data[sid]
    N = length(d.t_s)
    for i_start in 1 : SEGMENT_LENGTH-1 : N-1   # stride 9 for overlap-by-1
        i_end = min(i_start + SEGMENT_LENGTH - 1, N)
        i_end - i_start < 2 && continue   # skip degenerate tail
        push!(all_train_segments, (sid=sid, i_start=i_start, i_end=i_end))
    end
end
println("Total training segments: $(length(all_train_segments))")
println("Batch size:              $BATCH_SIZE  ($(round(100*BATCH_SIZE/length(all_train_segments), digits=1))% of total per step)")

# ============================================================================
# TRAINING LOSS — MULTI-SHOOTING WITH MINIBATCH SAMPLING (v66)
#
# Each gradient step samples a random subset of `BATCH_SIZE` segments from
# the pre-computed `all_train_segments` list. This implements Wang's
# `batch_size=512` configuration as a true stochastic minibatch instead
# of full-batch gradient descent over all ~1138 segments.
#
# Each segment is a 10-timestep window starting from a measured state,
# integrated forward by the hybrid Romero+NN ODE. Huber loss is computed
...
#
# Multi-shooting decomposes long-horizon optimization (28+ steps) into
...
#
# `Zygote.@ignore` on the random sampling prevents Zygote from attempting
# to differentiate through the integer index sampling, which would crash.
#
...
function loss_multishot(θ)
    ...
    # Random subset of segments for this gradient step. Zygote.@ignore
    # ensures the random sampling is not differentiated through.
    n_total = length(all_train_segments)
    bsz = min(BATCH_SIZE, n_total)
    batch_idx = Zygote.@ignore Random.shuffle(1:n_total)[1:bsz]

    for idx in batch_idx
        seg = all_train_segments[idx]
        sid = seg.sid
        i_start = seg.i_start
        i_end = seg.i_end
        ...
        # Initial condition is the MEASURED state at this segment's start
        u0_seg = T[d.li[i_start], d.ip_MA[i_start], d.V[i_start]]
        t_seg  = d.t_s[i_start : i_end]
...
CSV.write("predictions_RomeroNN_v66.csv", results_df)
println("Saved diagnostic predictions to predictions_RomeroNN_v66.csv")
...
CSV.write("RomeroNN_predictions_hard_shot_v66.csv", hard_df)
println("Saved hard-shot evaluation to RomeroNN_predictions_hard_shot_v66.csv")
...
savefig(plt_li, "li_RomeroNN_comparison_v66.png")
...
savefig(plt_ip, "ip_RomeroNN_comparison_v66.png")
...
savefig(plt_V, "V_RomeroNN_comparison_v66.png")
