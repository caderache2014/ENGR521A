# ============================================================================
# RomeroNN_v70.jl — Hybrid neural ODE for plasma current and inductance
...
#
# v70 change vs v69:
#   V is now included in the loss function alongside Li and Ip. Channel
#   contributions are normalized by per-channel variance computed over the
#   train split, so each of (Li, Ip, V) contributes ~equally to the gradient
#   regardless of magnitude differences. The previously unsupervised V
#   channel is now constrained by data; this experiment tests whether the
#   V_dot diagnostic R² (R² ≈ 0.007 in v69, where V was unsupervised)
#   improves when V is in the loss.
#
# v69 changes vs v68 (preserved):
#   1. BSON serialization of θ_trained + metadata for downstream analysis
#      (V_dot diagnostic, SR/SINDy work).
...
function calculate_df_norm(df_489_shots::DataFrame)
    norms = Float64[]
    for sid in unique(df_489_shots.shot)
        ...
        n = nrow(shot_rows)
        for i in 2:n
            dt = shot_rows.time[i] - shot_rows.time[i-1]
            dLi_dt = (shot_rows.li[i] - shot_rows.li[i-1]) / dt
            # Use the PREVIOUS timestep's Vind, V, Ip (matches Tino's .shift(1))
            numerator   = -2*shot_rows.Vind[i-1] - 2*shot_rows.vc_minus_vb[i-1]
            denominator = shot_rows.ip_MA[i-1] * dLi_dt
            if abs(denominator) > 1e-5
                push!(norms, numerator / denominator)
            end
        end
    end
    final_norm = median(norms)
    println("Auto-calculated empirical norm: $(round(final_norm, digits=4))")
    return final_norm
end

# Run it once and print
empirical_norm = calculate_df_norm(df_489_shots)
println("Empirical norm value: $empirical_norm")

# Also print some distribution stats so we can see how robust the median is
using Statistics
norms_temp = Float64[]
for sid in unique(df_489_shots.shot)
    shot_rows = df_489_shots[df_489_shots.shot .== sid, :]
    sort!(shot_rows, :time)
    for i in 2:nrow(shot_rows)
        dt = shot_rows.time[i] - shot_rows.time[i-1]
        dLi_dt = (shot_rows.li[i] - shot_rows.li[i-1]) / dt
        numerator   = -2*shot_rows.Vind[i-1] - 2*shot_rows.vc_minus_vb[i-1]
        denominator = shot_rows.ip_MA[i-1] * dLi_dt
        if abs(denominator) > 1e-5
            push!(norms_temp, numerator / denominator)
        end
    end
end
println("\n=== Empirical norm distribution ===")
println("count:    $(length(norms_temp))")
println("median:   $(median(norms_temp))")
println("mean:     $(mean(norms_temp))")
println("std:      $(std(norms_temp))")
println("25th pct: $(quantile(norms_temp, 0.25))")
println("75th pct: $(quantile(norms_temp, 0.75))")
println("99th pct: $(quantile(norms_temp, 0.99))")


max_ip_MA   = maximum(abs.(T.(df_489_shots.ip_MA)))
max_li      = maximum(abs.(T.(df_489_shots.li)))
#liip_norm   = T(max_ip_MA * max_li)  #version <= 62

# Empirical norm from data (median over all shot finite differences) ≈ 0.22.
# Hardcoded value 0.85 follows Tino's PyTorch default, which produced
# results close to Wang. The 4x discrepancy between empirical and default
# is unresolved; revisit if v63 results don't match Tino.
liip_norm    = T(0.85)   # added for v63
κ_empirical = T(0.98)
τ           = T(1.25)
...
function romero_ODEfunc!(du::AbstractVector{T}, u::AbstractVector{T}, p, t::T)
    ...
    dli_dt_denom = p.liip_norm * Ip + T(1e-8)  # signed Ip, small additive epsilon
    dip_dt_denom = p.liip_norm * Li + T(1e-8)   # signed Li, small additive epsilon
...
romero_NN = Lux.Chain(
    ...
    Lux.Dense(32, 1)        #identity output, unbounded
)
...
function dudt_romeroNN_diag(du, u, θ, t)
    Li, Ip, V = u
    Vind = Vind_spline32(t)
    du[1] = (-2*Vind - 2*V) / (liip_norm * Ip + T(1e-8))     # signed Ip, small additive epsilon
    du[2] = ( 2*Vind +    V) / (liip_norm * Li + T(1e-8))   # signed Li, small additive epsilon
    ps = re(θ)
...
# ============================================================================
# CHANNEL-VARIANCE PRECOMPUTE (v70)
#
# For v70's V-in-loss training, each channel's MSE term is divided by the
# channel's training-set variance. This normalizes the contribution of each
# of (Li, Ip, V) to the total loss so they contribute ~equally to the
# gradient regardless of magnitude differences (Li range ~0.6, Ip range
# ~0.3, V range ~2.3+). Variances are computed once over all training-set
# timesteps (concatenated across all train shots) and used as constants in
# the loss functions below. Saved into ckpt_meta for reproducibility.
# ============================================================================
const var_li = Float32(var(reduce(vcat, [shot_data[sid].li    for sid in train_shot_ids])))
const var_ip = Float32(var(reduce(vcat, [shot_data[sid].ip_MA for sid in train_shot_ids])))
const var_V  = Float32(var(reduce(vcat, [shot_data[sid].V     for sid in train_shot_ids])))

println("\n=== v70 channel variances (computed over $(length(train_shot_ids)) train shots) ===")
println("  var_li = $var_li")
println("  var_ip = $var_ip")
println("  var_V  = $var_V")
...
# v70 V-in-loss weighting. Set once; saved to ckpt_meta. The chosen value
# (0.01) reflects the order-of-magnitude scale difference between Wang's
# Huber-on-relative-error term (typical magnitude ~0.005 for Li/Ip residuals
# under 10% relative error) and the variance-normalized V residual term
# (typical magnitude ~0.5 for V residuals at half a standard deviation).
# This is a pragmatic default; a hyperparameter sweep could refine it.
const lambda_V = 0.01f0

function loss_multishot(θ)
    total_loss = 0.0f0
    n_valid_segments = 0
    segment_length = 10   # Wang's group_size
    sqrt_var_V = sqrt(var_V)   # precompute outside the loop
    ...
    for sid in train_shot_ids
        ...
        for i_start in seg_starts
            ...
            if sol.retcode == ReturnCode.Success
                ...
                V_seg  = d.V[i_start : i_end]
                # Relative error for Li, Ip (matches Wang's loss).
                r1 = (li_seg .- pred[1, :]) ./ (li_seg .+ sign.(li_seg) .* eps_l)
                r2 = (ip_seg .- pred[2, :]) ./ (ip_seg .+ sign.(ip_seg) .* eps_l)
                # v70: variance-normalized residual for V (avoids the
                # near-zero denominator problem of relative error since V
                # passes through zero in early-shot dynamics).
                r3 = (V_seg .- pred[3, :]) ./ sqrt_var_V
                # Inline Huber with δ = 0.1 (matches Wang's outlier_threshold).
                hub(x) = x ≤ T(0.1) ? T(0.5) * x^2 / T(0.1) : x - T(0.05)
                total_loss += sum(map(hub, r1) .+ map(hub, r2) .+ lambda_V .* map(hub, r3))
                n_valid_segments += 1
            end
        end
    end
...
function val_loss(θ)
    total_loss = 0.0f0
    n_valid    = 0
    sqrt_var_V = sqrt(var_V)   # precompute outside the loop (v70)
    ...
    for sid in val_shot_ids
        ...
        if sol.retcode == ReturnCode.Success
            ...
            # v70: variance-normalized V residual (V passes through zero,
            # so relative-error denominators are unstable for this channel).
            # Same lambda_V weighting as loss_multishot to keep train and
            # validation losses on comparable scales.
            r3 = abs.(d.V .- pred[3, :]) ./ sqrt_var_V
            hub(x) = x ≤ T(0.1) ? T(0.5) * x^2 / T(0.1) : x - T(0.05)
            total_loss += sum(map(hub, r1) .+ map(hub, r2) .+ lambda_V .* map(hub, r3)) / length(d.t_s)
            n_valid    += 1
        end
    end
...
#   κ_empirical, τ   - Romero physics constants (0.98, 1.25)
...
#                      verbatim Project.toml / Manifest.toml contents (so the
#                      exact dependency graph used at training time can be
#                      reconstructed if needed for the diagnostic script).
#
...
#   re-defines romero_NN with the same architecture (4 → 32 → 32 → 1,
...
#   using BSON
#   ckpt = BSON.load("romeroNN_checkpoint_v70.bson")
#   θ_trained = ckpt[:θ_trained]
#   # Re-define romero_NN identically, then:
#   _, re = destructure(Lux.setup(rng, romero_NN)[1])
#   ps = re(θ_trained)
# ============================================================================
...
ckpt_meta = (
    architecture     = "Chain(Dense(4,32,softplus), Dense(32,32,softplus), Dense(32,1))",
...
BSON.@save CHECKPOINT_PATH θ_trained liip_norm κ_empirical τ train_shot_ids val_shot_ids test_shot_ids ckpt_meta
...
function evaluate_split(shot_id_list, split_name::String)
    ...
    for sid in shot_id_list
        ...
        function romero_ODEfunc_f64!(du, u, p, t)
            ...
            eps64 = 1e-9
            dli_denom = p.liip_norm * Ip + 1e-8 # signed Ip, small additive epsilon
            dip_denom = p.liip_norm * Li + 1e-8 # signed Li, small additive epsilon
...
#
# IMPORTANT: dudt_eval_phil is the SEVENTH copy of the Romero physics
# formulation in this file. Must stay in sign-and-scaling sync with all
# other dudt closures.
# ============================================================================
function evaluate_split_endpoint(shot_id_list, split_name::String;
    ...
    for sid in shot_id_list
        ...
        for s0 in start_indices_0
            ...
            u0_ep = T[d.li[ep_idx_start],
                      d.ip_MA[ep_idx_start],
                      d.V[ep_idx_start]]

            prob_ep = ODEProblem(dudt_eval_phil, u0_ep,
                                 (ep_t[1], ep_t[end]), θ_trained)
            sol_ep  = solve(prob_ep, Tsit5();
                            p = θ_trained, saveat = ep_t,
                            reltol = 1f-4, abstol = 1f-6, verbose = false)
            ...
            if sol_ep.retcode == ReturnCode.Success
                ...
                push!(nn_li_endpoint_errors,
                      100f0 * abs(pred_ep[1, end] - li_target) / abs(li_target))
                push!(nn_ip_endpoint_errors,
                      100f0 * abs(pred_ep[2, end] - ip_target) / abs(ip_target))
            else
                n_failed_nn += 1
            end
        end
...
CSV.write("fullshot_mape_TEST_RomeroNN_v70.csv", fullshot_test_df)
println("Saved per-shot full-shot MAPE (TEST) to fullshot_mape_TEST_RomeroNN_v70.csv")
...
CSV.write("endpoint_mape_TEST_RomeroNN_v70.csv", endpoint_test_df)
println("Saved per-episode endpoint MAPE (TEST) to endpoint_mape_TEST_RomeroNN_v70.csv")
...
#     For RomeroNN, Li_dot and Ip_dot come from pure Romero physics
#     (this model does not augment those channels). Only V_dot is NN-augmented.
#   - Finite-difference derivatives from data, computed via Dierckx splines
#     at three smoothing levels: s=0, s=1e-4*N, s=1e-3*N (matches Vind).
#     Three FD columns per channel × three channels = 9 FD columns.
#
# Powers: V_dot diagnostic, SR Scenario B prep, SINDy on any channel,
# per-shot breakdown.
#
# IMPORTANT: V_dot_NN computation here is the EIGHTH location in this file
# where the Romero physics formula appears. Stays in sign-and-scaling sync
# with all other dudt closures.
# ============================================================================
...
col_Li_dot_s0    = Vector{Float32}(undef, 0); sizehint!(col_Li_dot_s0, n_rows_estimate)
...
col_Ip_dot_s0    = Vector{Float32}(undef, 0); sizehint!(col_Ip_dot_s0, n_rows_estimate)
...
col_V_dot_s0     = Vector{Float32}(undef, 0); sizehint!(col_V_dot_s0, n_rows_estimate)
...
for sid in shot_ids
    ...
    # Build per-channel FD splines at three smoothing levels.
    Nf = Float64(Tsh)
    spl_Li_s0    = Spline1D(d.t_s, d.li;    k=3, bc="extrapolate", s=0.0)
    spl_Li_s1em4 = Spline1D(d.t_s, d.li;    k=3, bc="extrapolate", s=1e-4 * Nf)
    spl_Li_s1em3 = Spline1D(d.t_s, d.li;    k=3, bc="extrapolate", s=1e-3 * Nf)
    spl_Ip_s0    = Spline1D(d.t_s, d.ip_MA; k=3, bc="extrapolate", s=0.0)
    spl_Ip_s1em4 = Spline1D(d.t_s, d.ip_MA; k=3, bc="extrapolate", s=1e-4 * Nf)
    spl_Ip_s1em3 = Spline1D(d.t_s, d.ip_MA; k=3, bc="extrapolate", s=1e-3 * Nf)
    spl_V_s0     = Spline1D(d.t_s, d.V;     k=3, bc="extrapolate", s=0.0)
    spl_V_s1em4  = Spline1D(d.t_s, d.V;     k=3, bc="extrapolate", s=1e-4 * Nf)
    spl_V_s1em3  = Spline1D(d.t_s, d.V;     k=3, bc="extrapolate", s=1e-3 * Nf)
    ...
    for i in 1:Tsh
        ...
        Li_dot_NN_i = (-2*Vind_i - 2*V_i) / (liip_norm * Ip_i + T(1e-8))
        Ip_dot_NN_i = ( 2*Vind_i +    V_i) / (liip_norm * Li_i + T(1e-8))
        V_dot_NN_i  = -V_i/τ - (κ_empirical/τ)*Vind_i + nn_out[1]
...
derivatives_df = DataFrame(
    ...
    Li_dot_FD_s0    = col_Li_dot_s0,
    Li_dot_FD_s1em4 = col_Li_dot_s1em4,
    Li_dot_FD_s1em3 = col_Li_dot_s1em3,
    Ip_dot_FD_s0    = col_Ip_dot_s0,
    Ip_dot_FD_s1em4 = col_Ip_dot_s1em4,
    Ip_dot_FD_s1em3 = col_Ip_dot_s1em3,
    V_dot_FD_s0     = col_V_dot_s0,
    V_dot_FD_s1em4  = col_V_dot_s1em4,
    V_dot_FD_s1em3  = col_V_dot_s1em3,
)
CSV.write("derivatives_RomeroNN_v70.csv", derivatives_df)
println("Saved derivatives CSV: $(nrow(derivatives_df)) rows × $(ncol(derivatives_df)) cols → derivatives_RomeroNN_v70.csv")
...
CSV.write("predictions_RomeroNN_v70.csv", results_df)
println("Saved diagnostic predictions to predictions_RomeroNN_v70.csv")
...
CSV.write("loss_history_RomeroNN_v70.csv", loss_df)
println("Saved training loss history to loss_history_RomeroNN_v70.csv")

val_df = DataFrame(
    iter     = [vh[1] for vh in val_history],
    val_loss = [vh[2] for vh in val_history],
)
CSV.write("val_loss_history_RomeroNN_v70.csv", val_df)
println("Saved validation loss history to val_loss_history_RomeroNN_v70.csv")
...
plot!(plt_loss, val_df.iter, val_df.val_loss,
      ...
      seriestype = :scatter,
      ms    = 4)
savefig(plt_loss, "loss_history_RomeroNN_v70.png")
println("Saved loss-history plot to loss_history_RomeroNN_v70.png")
...
CSV.write("RomeroNN_predictions_hard_shot_v70.csv", hard_df)
println("Saved hard-shot evaluation to RomeroNN_predictions_hard_shot_v70.csv")
...
savefig(plt_li, "li_RomeroNN_comparison_v70.png")
...
savefig(plt_ip, "ip_RomeroNN_comparison_v70.png")
...
savefig(plt_V, "V_RomeroNN_comparison_v70.png")
